...
//        Flux<Integer> numbers = Flux.concat(numberService1(), numberService2());
    ...
    public void mail_box_switcher() {
        //todo: feel free to change code as you need
        Flux<Message> myMail = mailBoxPrimary().map(message -> {
            if (message.metaData.equals("spam")) {
                throw new RuntimeException();
            }

            return message;
        }).onErrorResume(throwable -> mailBoxSecondary());
    ...
    public void prettify() {
        ...
        Mono<Boolean> successful = Mono.when(openFile()).and(writeToFile("0x3522285912341"))
                .then(closeFile()).then(Mono.just(true));
    ...
    public void acid_durability() {
        //todo: feel free to change code as you need
        Flux<String> committedTasksIds =  tasksToExecute().flatMapSequential(stringMono -> stringMono.map(s -> {
            commitTask(s);
            return s;
        }));
    ...
    public Mono<String> chooseSource() {
        return Mono.empty().publish(objectMono -> {
    ...
    public void cleanup() {
        ...
        Flux<String> stream = StreamingConnection.startStreaming()
                .flatMapMany(Function.identity()).doOnTerminate(() -> StreamingConnection.closeConnection().subscribe());
